...
// GET --> Muestra los anuncios
router.get("/", (req, res, next) => {
  const start = parseInt(req.query.start) || 0;
  const limit = parseInt(req.query.limit) || 10; // api devuelve max 10 registros
...
// GET /api/anuncio/findbyusername/username
...
// PUT /api/anuncios/addfav/idusuario/idanuncio --> Añade fav y quita
router.put("/addfav/:idusuario/:idanuncio", async (req, res, next) => {
  try {
    const idUser = req.params.idusuario;
    const idAnuncio = req.paramns.idanuncio;

    const anuncio = await Anuncio.findOne({ _id: idAnuncio });

    if (!anuncio.favs.include(idUser)) {
      anuncio.favs.push(idUser);
    } else {
      var i = anuncio.favs.indexOf(idUser);
      i !== -1 && anuncio.favs.splice(i, 1);
    }
    const anuncioFav = await Anuncio.findOneAndUpdate(
      { _id: idAnuncio },
      anuncio
    );
    res.json({ result: anuncioFav });
  } catch (err) {
    next(err);
  }
});

// GET /api/anuncio/fav/:idusuario --> Muestra los favs de un user
router.get("/fav/:idusuario", async (req, res, next) => {
  try {
    const idusuario = req.params.idusuario;

    const anuncio = await Anuncio.findOne({ id: idusuario });
    if (!anuncio) {
      const err = new Error("not found");
      err.status = 404;
      return next(err);
...
// PUT /api/anuncios/estado --> marca y desmarca como vendido
